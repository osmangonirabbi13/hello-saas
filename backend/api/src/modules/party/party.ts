...
export type PartyRepositoryContract = {
  create(kind: PartyKind, businessId: string, userId: string, input: PartyInput): Promise<object>;
...
export class PartyRepository implements PartyRepositoryContract {
  async create(kind: PartyKind, businessId: string, userId: string, input: PartyInput) {
  ...
  async update(kind: PartyKind, businessId: string, id: string, input: Partial<PartyInput>) {
...
export class PartyService {
  constructor(
    private readonly kind: PartyKind,
    private readonly repository: PartyRepositoryContract,
  ) {}
  async create(businessId: string, userId: string, input: PartyInput) {
    try {
      return await this.repository.create(this.kind, businessId, userId, input);
    } catch (error) {
  ...
  async update(businessId: string, id: string, input: Partial<PartyInput>) {
    const item = await this.repository.update(this.kind, businessId, id, input);
    if (!item) throw new AppError(404, 'PARTY_NOT_FOUND', 'Record was not found.');
    return item;
  }
...
export function partyController(service: PartyService) {
  ...
  const create: RequestHandler = (req, res, next) => {
    void service
      .create(req.tenant!.businessId, req.auth!.id, req.body as PartyInput)
  ...
  const update: RequestHandler = (req, res, next) => {
    void service
      .update(req.tenant!.businessId, String(req.params.id), req.body as PartyInput)
      .then((data) => success(res, data))
      .catch(next);
  };
